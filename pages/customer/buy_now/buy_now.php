...
$sql_cart = "SELECT c.cart_id, c.quantity, p.product_id, p.product_name, p.price, p.stock, p.image_id
             FROM cart c
    ...
    if (!empty($house_no) && !empty($street1) && !empty($city) && !empty($postal_code) && !empty($payment_method)) {
        ...
        try {
            ...
            foreach ($cart_items as $item) {
                ...
                // Reduce stock
                $stmt_update = $conn->prepare("UPDATE product SET stock = stock - ? WHERE product_id = ?");
            ...
            $stmt_clear->bind_param("i", $userID);
            $stmt_clear->execute();
    ...
    <div class="checkout-container">
        ...
        <div class="order-summary">
            ...
            <?php foreach ($cart_items as $item): ?>
                <div class="cart-item">
                    <img src="../../../assets/products/product_<?php echo htmlspecialchars($item['image_id'] ?? 'default'); ?>.png"
